var Godot = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    function findWasmBinary() {
      return locateFile("godot.web.template_release.wasm32.wasm");
    }
    ...
    var PThread = {
      ...
      initMainThread() {
        var pthreadPoolSize = Module["emscriptenPoolSize"] || 8;
    ...
    function ___syscall_getdents64(fd, dirp, count) {
      ...
      try {
        ...
        for (var idx = startIdx; idx < endIdx; idx++) {
          ...
          (growMemViews(), HEAP64)[(dirp + pos + 8) >> 3] = BigInt(
            (idx + 1) * struct_size,
          );
    ...
    function ___syscall_mknodat(dirfd, path, mode, dev) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(15, 0, 1, dirfd, path, mode, dev);
      try {
        path = SYSCALLS.getStr(path);
        path = SYSCALLS.calculateAt(dirfd, path);
        switch (mode & 61440) {
    ...
    function ___syscall_newfstatat(dirfd, path, buf, flags) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(16, 0, 1, dirfd, path, buf, flags);
      try {
        path = SYSCALLS.getStr(path);
    ...
    function ___syscall_openat(dirfd, path, flags, varargs) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(17, 0, 1, dirfd, path, flags, varargs);
      SYSCALLS.varargs = varargs;
      try {
        path = SYSCALLS.getStr(path);
        path = SYSCALLS.calculateAt(dirfd, path);
    ...
    var GL = {
      ...
      getSource: (shader, count, string, length) => {
        ...
        for (var i = 0; i < count; ++i) {
          var len = length
            ? (growMemViews(), HEAPU32)[(length + i * 4) >> 2]
            : undefined;
          source += UTF8ToString(
            (growMemViews(), HEAPU32)[(string + i * 4) >> 2],
            len,
          );
        }
        return source;
    ...
    function __gmtime_js(time, tmPtr) {
      ...
      (growMemViews(), HEAP32)[(tmPtr + 20) >> 2] =
    ...
    function __localtime_js(time, tmPtr) {
      ...
      (growMemViews(), HEAP32)[(tmPtr + 28) >> 2] = yday;
      (growMemViews(), HEAP32)[(tmPtr + 36) >> 2] = -(
    ...
    function __setitimer_js(which, timeout_ms) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(25, 0, 1, which, timeout_ms);
      if (timers[which]) {
    ...
    var _glBufferData = (target, size, data, usage) => {
      ...
      GLctx.bufferData(
        target,
        data ? (growMemViews(), HEAPU8).subarray(data, data + size) : size,
        usage,
      );
    };
    ...
    var _glBufferSubData = (target, offset, size, data) => {
      ...
      GLctx.bufferSubData(
        target,
        offset,
        (growMemViews(), HEAPU8).subarray(data, data + size),
      );
    };
    ...
    var _glCompressedTexImage2D = (
      ...
      GLctx.compressedTexImage2D(
        ...
        height,
        border,
        (growMemViews(), HEAPU8).subarray(data, data + imageSize),
      );
    };
    ...
    var readI53FromI64 = (ptr) =>
      (growMemViews(), HEAPU32)[ptr >> 2] +
      (growMemViews(), HEAP32)[(ptr + 4) >> 2] * 4294967296;
    var readI53FromU64 = (ptr) =>
      (growMemViews(), HEAPU32)[ptr >> 2] +
      (growMemViews(), HEAPU32)[(ptr + 4) >> 2] * 4294967296;
    var writeI53ToI64 = (ptr, num) => {
      ...
      if (deserialized != num)
        warnOnce(
          `writeI53ToI64() out of range: serialized JS Number ${num} to Wasm heap as bytes lo=${ptrToString((growMemViews(), HEAPU32)[offset])}, hi=${ptrToString((growMemViews(), HEAPU32)[offset + 1])}, which deserializes back to ${deserialized} instead!`,
        );
    };
    ...
    var _glGetProgramInfoLog = (program, maxLength, length, infoLog) => {
      ...
      if (length)
        (growMemViews(), HEAP32)[length >> 2] = numBytesWrittenExclNull;
    };
    ...
    var _glGetShaderiv = (shader, pname, p) => {
      ...
      if (pname == 35716) {
        ...
        (growMemViews(), HEAP32)[p >> 2] = GLctx.getShaderParameter(
          GL.shaders[shader],
          pname,
        );
      }
    ...
    var _glTransformFeedbackVaryings = (
      ...
      for (var i = 0; i < count; i++)
        vars.push(
          UTF8ToString((growMemViews(), HEAP32)[(varyings + i * 4) >> 2]),
        );
    ...
    var _glUniform1iv = (location, count, value) => {
      ...
      if (count <= 288) {
        ...
        var view = (growMemViews(), HEAP32).subarray(
          value >> 2,
          (value + count * 4) >> 2,
        );
      }
    ...
    var _glUniform2fv = (location, count, value) => {
      ...
      if (count <= 144) {
        ...
        var view = (growMemViews(), HEAPF32).subarray(
          value >> 2,
          (value + count * 8) >> 2,
        );
      }
    ...
    function setCanvasElementSizeMainThread(target, width, height) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(27, 0, 1, target, width, height);
    ...
    function _emscripten_webgl_destroy_context(contextHandle) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(28, 0, 1, contextHandle);
    ...
    function _emscripten_webgl_create_context_proxied(target, attributes) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(29, 0, 1, target, attributes);
    ...
    var _emscripten_webgl_do_create_context = (target, attributes) => {
      ...
      var contextAttributes = {
        ...
        failIfMajorPerformanceCaveat: !!(growMemViews(), HEAP8)[
          attributes + 12
        ],
    ...
    function _emscripten_webgl_enable_extension(contextHandle, extension) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(30, 0, 1, contextHandle, extension);
    ...
    function _environ_get(__environ, environ_buf) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(32, 0, 1, __environ, environ_buf);
      var bufSize = 0;
      var envp = 0;
    ...
    function _environ_sizes_get(penviron_count, penviron_buf_size) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(33, 0, 1, penviron_count, penviron_buf_size);
    ...
    function _fd_read(fd, iov, iovcnt, pnum) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(36, 0, 1, fd, iov, iovcnt, pnum);
      try {
        var stream = SYSCALLS.getStreamFromFD(fd);
    ...
    function _fd_seek(fd, offset, whence, newOffset) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(37, 0, 1, fd, offset, whence, newOffset);
    ...
    var GodotRuntime = {
      ...
      parseStringArray: function (p_ptr, p_size) {
        const strings = [];
        const ptrs = GodotRuntime.heapSub(
          (growMemViews(), HEAP32),
          p_ptr,
          p_size,
        );
      ...
      allocStringArray: function (p_strings) {
        ...
        for (let i = 0; i < size; i++) {
          (growMemViews(), HEAP32)[(c_ptr >> 2) + i] = GodotRuntime.allocString(
            p_strings[i],
          );
        }
        return c_ptr;
      ...
      freeStringArray: function (p_ptr, p_len) {
        for (let i = 0; i < p_len; i++) {
          GodotRuntime.free((growMemViews(), HEAP32)[(p_ptr >> 2) + i]);
        }
        GodotRuntime.free(p_ptr);
      },
      stringToHeap: function (p_str, p_ptr, p_len) {
        return stringToUTF8Array(p_str, (growMemViews(), HEAP8), p_ptr, p_len);
      },
    };
    var GodotConfig = {
      canvas: null,
    ...
    function _godot_audio_get_sample_playback_position(playbackObjectIdStrPtr) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(39, 0, 1, playbackObjectIdStrPtr);
    ...
    function _godot_audio_sample_bus_move(fromPos, toPos) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(48, 0, 1, fromPos, toPos);
    ...
    function _godot_audio_sample_set_finished_callback(callbackPtr) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(57, 0, 1, callbackPtr);
    ...
    function _godot_audio_sample_set_pause(playbackObjectIdStrPtr, pause) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(58, 0, 1, playbackObjectIdStrPtr, pause);
      const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
    ...
    function _godot_audio_sample_set_volumes_linear(
      ...
      const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
      const buses = GodotRuntime.heapSub(
        (growMemViews(), HEAP32),
        busesPtr,
        busesSize,
      );
    ...
    function _godot_audio_sample_start(
      ...
      const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
      const streamObjectId = GodotRuntime.parseString(streamObjectIdStrPtr);
      const volume = GodotRuntime.heapSub(
        (growMemViews(), HEAPF32),
        volumePtr,
        8,
      );
    ...
    function _godot_audio_sample_unregister_stream(streamObjectIdStrPtr) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(63, 0, 1, streamObjectIdStrPtr);
      const streamObjectId = GodotRuntime.parseString(streamObjectIdStrPtr);
    ...
    function _godot_audio_sample_update_pitch_scale(
      ...
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(64, 0, 1, playbackObjectIdStrPtr, pitchScale);
      const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
    ...
    function _godot_audio_script_create(buffer_length, channel_count) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(65, 0, 1, buffer_length, channel_count);
    ...
    function _godot_audio_worklet_start(
      ...
      const state = GodotRuntime.heapSub((growMemViews(), HEAP32), p_state, 4);
    ...
    function _godot_audio_worklet_state_get(p_state, p_idx) {
      return Atomics.load((growMemViews(), HEAP32), (p_state >> 2) + p_idx);
    }
    ...
    function _godot_js_display_cursor_set_custom_shape(
      ...
      if (p_len > 0) {
        const png = new Blob(
          [GodotRuntime.heapSlice((growMemViews(), HEAPU8), p_ptr, p_len)],
          { type: "image/png" },
        );
    ...
    function _godot_js_display_desired_size_set(width, height) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(82, 0, 1, width, height);
    ...
    function _godot_js_display_screen_size_get(width, height) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(91, 0, 1, width, height);
      const scale = GodotDisplayScreen.getPixelRatio();
    ...
    function _godot_js_display_vk_cb(p_input_cb) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(97, 0, 1, p_input_cb);
    ...
    function _godot_js_display_vk_show(p_text, p_type, p_start, p_end) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(99, 0, 1, p_text, p_type, p_start, p_end);
      const text = GodotRuntime.parseString(p_text);
    ...
    function _godot_js_display_window_icon_set(p_ptr, p_len) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(101, 0, 1, p_ptr, p_len);
    ...
    function _godot_js_display_window_size_get(p_width, p_height) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(102, 0, 1, p_width, p_height);
      GodotRuntime.setHeapValue(p_width, GodotConfig.canvas.width, "i32");
      GodotRuntime.setHeapValue(p_height, GodotConfig.canvas.height, "i32");
    }
    function _godot_js_display_window_title_set(p_data) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(103, 0, 1, p_data);
      ...
      switch (typeof eval_ret) {
        ...
        case "object":
          ...
          if (eval_ret instanceof Uint8Array) {
            ...
            (growMemViews(), HEAPU8).set(eval_ret, bytes_ptr);
            return 29;
          }
          break;
      }
    ...
    function _godot_js_fetch_create(
      ...
      const body = p_body_size
        ? GodotRuntime.heapSlice((growMemViews(), HEAP8), p_body, p_body_size)
        : null;
    ...
    function _godot_js_fetch_read_chunk(p_id, p_buf, p_buf_size) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(109, 0, 1, p_id, p_buf, p_buf_size);
      const obj = IDHandler.get(p_id);
      if (!obj || !obj.response) {
        return 0;
      }
      ...
      while (to_read && chunks.length) {
        ...
        if (chunk.length > to_read) {
          ...
          GodotRuntime.heapCopy((growMemViews(), HEAP8), chunk, p_buf);
    ...
    function _godot_js_fetch_read_headers(p_id, p_parse_cb, p_ref) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(110, 0, 1, p_id, p_parse_cb, p_ref);
      const obj = IDHandler.get(p_id);
      if (!obj || !obj.response) {
        return 1;
      }
    ...
    function _godot_js_input_gamepad_cb(change_cb) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(113, 0, 1, change_cb);
    ...
    function _godot_js_input_key_cb(callback, code, key) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(117, 0, 1, callback, code, key);
      const func = GodotRuntime.get_func(callback);
    ...
    function _godot_js_input_touch_cb(callback, ids, coords) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(122, 0, 1, callback, ids, coords);
      const func = GodotRuntime.get_func(callback);
      const canvas = GodotConfig.canvas;
    ...
    function _godot_js_input_vibrate_handheld(p_duration_ms) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(123, 0, 1, p_duration_ms);
    ...
    function _godot_js_os_download_buffer(p_ptr, p_size, p_name, p_mime) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(125, 0, 1, p_ptr, p_size, p_name, p_mime);
      const buf = GodotRuntime.heapSlice(
        (growMemViews(), HEAP8),
        p_ptr,
        p_size,
      );
    ...
    function _godot_js_os_finish_async(p_callback) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(127, 0, 1, p_callback);
      const func = GodotRuntime.get_func(p_callback);
    ...
    function _godot_js_os_request_quit_cb(p_callback) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(132, 0, 1, p_callback);
    ...
    function _godot_js_os_thread_pool_size_get() {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(134, 0, 1);
    ...
    function _godot_js_pwa_cb(p_update_cb) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(135, 0, 1, p_update_cb);
    ...
    function _godot_js_set_ime_active(p_active) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(137, 0, 1, p_active);
      GodotIME.ime_active(p_active);
    }
    function _godot_js_set_ime_cb(p_ime_cb, p_key_cb, code, key) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(138, 0, 1, p_ime_cb, p_key_cb, code, key);
    ...
    function _godot_js_set_ime_position(p_x, p_y) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(139, 0, 1, p_x, p_y);
      GodotIME.ime_position(p_x, p_y);
    }
    function _godot_js_tts_get_voices(p_callback) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(140, 0, 1, p_callback);
      const func = GodotRuntime.get_func(p_callback);
      try {
        const arr = [];
    ...
    function _godot_js_tts_is_paused() {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(141, 0, 1);
    ...
    function _godot_js_webmidi_close_midi_inputs() {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(147, 0, 1);
        ...
        inputs.forEach((input, i) => {
          ...
          input.addEventListener(
            ...
            (event) => {
              ...
              (growMemViews(), HEAPU8).set(data, pDataBuffer);
    ...
    function _godot_js_wrapper_create_cb(p_ref, p_func) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(149, 0, 1, p_ref, p_func);
    ...
    function _godot_js_wrapper_interface_get(p_name) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(151, 0, 1, p_name);
      const name = GodotRuntime.parseString(p_name);
      if (typeof window[name] !== "undefined") {
    ...
    function _godot_js_wrapper_object_get(p_id, p_exchange, p_prop) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(153, 0, 1, p_id, p_exchange, p_prop);
      const obj = GodotJSWrapper.get_proxied_value(p_id);
      if (obj === undefined) {
        return 0;
      }
    ...
    function _godot_js_wrapper_object_getvar(p_id, p_type, p_exchange) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(154, 0, 1, p_id, p_type, p_exchange);
      const obj = GodotJSWrapper.get_proxied_value(p_id);
      if (obj === undefined) {
        return -1;
      }
    ...
    function _godot_js_wrapper_object_set(p_id, p_name, p_type, p_exchange) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(156, 0, 1, p_id, p_name, p_type, p_exchange);
      const obj = GodotJSWrapper.get_proxied_value(p_id);
      if (obj === undefined) {
        return;
      }
    ...
    function _godot_js_wrapper_object_set_cb_ret(p_val_type, p_val_ex) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(157, 0, 1, p_val_type, p_val_ex);
    ...
    function _godot_js_wrapper_object_transfer_buffer(
      ...
      (growMemViews(), HEAPU8).set(obj, bytesPtr);
    }
    function _godot_js_wrapper_object_unref(p_id) {
      if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(160, 0, 1, p_id);
    ...
    function _godot_webgl2_glGetBufferSubData(target, offset, size, data) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(163, 0, 1, target, offset, size, data);
      const gl_context_handle = _emscripten_webgl_get_current_context();
      const gl = GL.getContext(gl_context_handle);
      if (gl) {
    ...
    var proxiedFunctionTable = [
      ...
      _godot_audio_sample_bus_move,
      _godot_audio_sample_bus_remove,
      _godot_audio_sample_bus_set_count,
      _godot_audio_sample_bus_set_mute,
      _godot_audio_sample_bus_set_send,
      _godot_audio_sample_bus_set_solo,
      _godot_audio_sample_bus_set_volume_db,
      _godot_audio_sample_is_active,
      ...
      _godot_audio_sample_start,
      _godot_audio_sample_stop,
      ...
      _godot_js_emscripten_get_version,
      ...
      _godot_js_set_ime_active,
      _godot_js_set_ime_cb,
      _godot_js_set_ime_position,
    ...
    var _emscripten_webgl_get_current_context = makeInvalidEarlyAccess(
    ...
    var __emscripten_thread_crashed = (Module["__emscripten_thread_crashed"] =
      makeInvalidEarlyAccess("__emscripten_thread_crashed"));
    ...
    function assignWasmExports(wasmExports) {
      ...
      _emscripten_webgl_get_current_context = createExportWrapper(
    ...
    function assignWasmImports() {
      wasmImports = {
        ...
        godot_js_os_thread_pool_size_get: _godot_js_os_thread_pool_size_get,
    ...
    addOnPostRun(function () {
      GL.getSource = (shader, count, string, length) => {
        ...
        for (let i = 0; i < count; ++i) {
          const ptr = (growMemViews(), HEAPU32)[(string + i * 4) >> 2];
          const len = length
            ? (growMemViews(), HEAPU32)[(length + i * 4) >> 2]
            : undefined;
          if (len) {
            ...
            const slice =
              (growMemViews(), HEAPU8).buffer instanceof ArrayBuffer
                ? (growMemViews(), HEAPU8).subarray(ptr, endPtr)
                : (growMemViews(), HEAPU8).slice(ptr, endPtr);
